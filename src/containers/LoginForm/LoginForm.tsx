...
export default function () {
    ...
    const login = () => {
        ...
        ENDPOINTS.auth().login({ email: username, password })
        .then((response: any) => {
            ...
            if(response.data?.accessToken && response.data?.user?.admin) {
                ...
                setTimeout(() => {
                    ...
                    setCookie("refresh_token", { refreshToken: response.data?.refreshToken }, { expires })
                    setCookie("token", { accessToken: response.data?.accessToken }, { expires })
